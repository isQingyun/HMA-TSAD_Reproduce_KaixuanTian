...
class HMATSADAgent:
    ...
    def _run_single(self, sample: TimeSeriesSample) -> dict[str, Any]:
    ...
    def _aggregate_attention(
        windows_and_results: list[tuple[SeriesWindow, dict[str, Any]]],
    ) -> dict[str, Any]:
        counters = (
            "detail_visual_actions",
            "unique_focus_regions",
            "exact_repeat_actions",
            "same_tool_near_repeat_actions",
            "blocked_repeated_visual_actions",
            "novelty_redirects",
            "exploration_targets",
        ...
        for window, result in windows_and_results:
            summary = result.get("attention_summary", {})
            ...
            for key in counters:
                try:
                    ...
                    continue
            resolved.extend(
                map_local_intervals(window, summary.get("resolved_event_intervals", []))
            )
        actions = totals["detail_visual_actions"]
        focuses = totals["unique_focus_regions"]
        return {
            **totals,
            "actions_per_focus": round(actions / max(1, focuses), 6),
            "resolved_event_intervals": [
                item.to_list() for item in merge_intervals(resolved)
            ],
            "scope": "sum of independently processed slices",
        }

    def _run_windowed(self, sample: TimeSeriesSample) -> dict[str, Any]:
        windows, stride = plan_series_windows(
            sample.length,
            self.config.plot_max_points,
            self.config.slice_overlap_ratio,
        )
        manifest_path = self.run_dir / "slice_manifest.json"
        manifest: dict[str, Any] = {
            ...
            "sample_id": sample.sample_id,
            "series_length": sample.length,
            "window_size": min(self.config.plot_max_points, sample.length),
            "overlap_ratio": self.config.slice_overlap_ratio,
            "stride": stride,
            "completed_windows": 0,
            "window_count": len(windows),
            "windows": [
                {**window.to_dict(), "status": "pending"} for window in windows
            ],
        }
        save_json(manifest_path, manifest)
        self.trace.log("windowed_run_start", manifest)

        single_config = replace(self.config, enable_window_slicing=False)
        windows_and_results: list[tuple[SeriesWindow, dict[str, Any]]] = []
        windows_and_predictions: list[
            tuple[SeriesWindow, list[dict[str, Any]]]
        ] = []
        mapped_overview: list[Interval] = []
        mapped_observed: list[Interval] = []
        slice_summaries: list[dict[str, Any]] = []

        for window in windows:
            item = manifest["windows"][window.index]
            item["status"] = "running"
            save_json(manifest_path, manifest)
            slice_name = (
                f"{sample.sample_id}__slice_{window.index:04d}_"
                f"{window.interval.start}_{window.interval.end}"
            )
            slice_dir = (
                self.run_dir
                / "slices"
                / f"{window.index:04d}_{window.interval.start}_{window.interval.end}"
            )
            prediction_path = slice_dir / "predictions.json"
            reused = prediction_path.exists() and not self.force
            try:
                if reused:
                    slice_result = json.loads(prediction_path.read_text(encoding="utf-8"))
                else:
                    slice_sample = TimeSeriesSample(
                        dataset=sample.dataset,
                        sample_id=slice_name,
                        values=sample.values[
                            window.interval.start : window.interval.end + 1
                        ],
                        train_values=sample.train_values,
                    ...
                    slice_result = child._run_single(slice_sample)
            except Exception as exc:
                item["status"] = "failed"
                ...
                self.trace.log(
                    "window_failed",
                    {"window": window.to_dict(), "error": str(exc)},
                )
                raise

            raw_predictions = slice_result.get("predictions", [])
            if not isinstance(raw_predictions, list):
                raw_predictions = []
            windows_and_results.append((window, slice_result))
            windows_and_predictions.append((window, raw_predictions))
            mapped_overview.extend(
                map_local_intervals(window, slice_result.get("overview_intervals", []))
            )
            mapped_observed.extend(
                map_local_intervals(window, slice_result.get("observed_intervals", []))
            )
            summary = {
                **window.to_dict(),
                "sample_id": slice_name,
                ...
                "reused": reused,
                "detail_coverage": float(slice_result.get("detail_coverage", 0.0)),
                "overview_coverage": float(slice_result.get("overview_coverage", 0.0)),
                "attention_summary": slice_result.get("attention_summary", {}),
            }
            ...
            manifest["completed_windows"] = len(windows_and_results)
            save_json(manifest_path, manifest)
            self.trace.log("window_complete", summary)

        predictions, aggregation = aggregate_window_predictions(
            sample.length,
            windows_and_predictions,
            vote_threshold=self.config.slice_vote_threshold,
            vote_aggregation=self.config.slice_vote_aggregation,
            merge_gap=self.config.merge_gap,
        )
        overview_intervals = merge_intervals(mapped_overview)
        observed_intervals = merge_intervals(mapped_observed)
        attention_summary = self._aggregate_attention(windows_and_results)
        ...
        result = {
            ...
            "model": self.client.config.model,
            "series_length": sample.length,
            "normalization": "median/MAD robust standardization per slice",
            "reference_source": "recorded separately for each slice",
            "coverage": coverage_fraction(observed_intervals, sample.length),
            "detail_coverage": coverage_fraction(observed_intervals, sample.length),
            "overview_coverage": coverage_fraction(overview_intervals, sample.length),
            "predictions": predictions,
            "overview_intervals": [item.to_list() for item in overview_intervals],
            "observed_intervals": [item.to_list() for item in observed_intervals],
            "attention_summary": attention_summary,
            "windowing": {
                "enabled": True,
                "window_size_source": "agent.plot_max_points",
                "window_size": min(self.config.plot_max_points, sample.length),
                "overlap_ratio": self.config.slice_overlap_ratio,
                "stride": stride,
                "window_count": len(windows),
                "full_series_coverage": aggregation["covered_points"] / sample.length,
                "vote_threshold": self.config.slice_vote_threshold,
                "vote_aggregation": self.config.slice_vote_aggregation,
                "aggregation": (
                    f"point-wise {self.config.slice_vote_aggregation} confidence "
                    "over all covering slices"
                ),
                "manifest_path": str(manifest_path),
            },
            "slice_results": slice_summaries,
            "aggregation_diagnostics": aggregation,
            "label_policy": "ground-truth labels were not loaded during inference",
        }
        save_json(self.run_dir / "predictions.json", result)
        self.trace.log(
            "windowed_run_complete",
            {
                "window_count": len(windows),
                "prediction_count": len(predictions),
                "detail_coverage": result["detail_coverage"],
                "overview_coverage": result["overview_coverage"],
                "attention_summary": attention_summary,
    ...
    def run(self, sample: TimeSeriesSample) -> dict[str, Any]:
        if self.config.enable_window_slicing and sample.length > self.config.plot_max_points:
            return self._run_windowed(sample)
        return self._run_single(sample)
