...
from .state import AgentState, AnomalyMark, TraceLogger, interval_overlap_fraction
...
class ToolEnvironment:
    ...
    def _candidate_marks(
        ...
        source = f"{source}:origin={self.state.active_strategy or 'unknown'}"
    ...
    def _visual_action_repeated(
        ...
        for record in self.state.plot_history:
            if record.get("tool") != tool:
                continue
            if tool == "Plot" and resolution is not None and record.get("resolution") != resolution:
                continue
            key = "requested_interval" if tool == "Compare" else "interval"
            raw = record.get(key, record.get("interval", []))
            if not isinstance(raw, list) or len(raw) != 2:
                continue
            previous = Interval(int(raw[0]), int(raw[1])).clamp(len(self.series))
            if interval_overlap_fraction(interval, previous) >= self.config.focus_overlap_threshold:
                return True
        return False
    ...
    def plot(self, start: int, end: int) -> dict[str, Any]:
        ...
        if self._visual_action_repeated("Plot", interval, resolution):
            self.state.blocked_repeated_visual_actions += 1
            payload = {
                "status": "error",
                "tool": "Plot",
                "interval": interval.to_list(),
                "message": "Novelty guard rejected a repeated or strongly overlapping Plot focus.",
            }
            self.trace.log("tool_repeated_visual_rejected", payload)
            return payload
    ...
    def compare(self, start: int, end: int, reference_index: Any = -1) -> dict[str, Any]:
        ...
        if self._visual_action_repeated("Compare", requested_interval):
            self.state.blocked_repeated_visual_actions += 1
            payload = {
                "status": "error",
                "tool": "Compare",
                "interval": requested_interval.to_list(),
                "message": "Novelty guard rejected a repeated or strongly overlapping Compare focus.",
            }
            self.trace.log("tool_repeated_visual_rejected", payload)
            return payload
    ...
    def mark(
        ...
        latest_compare = next(
            (
                record
                for record in reversed(self.state.plot_history)
                if record.get("tool") == "Compare"
                and isinstance(record.get("interval"), list)
                and len(record["interval"]) == 2
                and Interval(*record["interval"]).start <= interval.start
                and interval.end <= Interval(*record["interval"]).end
            ),
            None,
        )
        if self.config.mode == "optimized" and latest_compare is not None:
            compare_name = Path(str(latest_compare.get("path", ""))).name
            compare_candidates = [
                candidate
                for candidate in self.state.analyzer_candidates
                if "detail:compare:" in candidate.source
                and (not compare_name or compare_name in candidate.source)
            ]
            if not any(interval.overlaps(candidate.interval) for candidate in compare_candidates):
                return {
                    "status": "error",
                    "tool": "Mark",
                    "message": (
                        "Mark rejected because this focus already has Compare evidence and the proposed "
                        "interval is not supported by that comparison"
                    ),
                }
