...
class HMATSADAgent:
    ...
    def _learn_reference_memory(
        ...
        numeric_catalog = build_reference_catalog(reference_series, reference_intervals)
        ...
        fallback = {
            ...
            "normal_pattern": (
                "Use the cross-reference consensus of the numeric profiles. Training references are designated normal; "
                "test-unsupervised references are fallible candidates and must not be treated as ground truth."
            ),
            "reference_profiles": fallback_profiles,
            "selection_guidance": "Prefer the reliable candidate with the lowest shape distance to the query.",
        ...
        for index, interval in enumerate(reference_intervals):
            ...
            render_interval(
                ...
                interval,
                path,
                f"Reference candidate {index} ({reference_source})",
    ...
    def run(self, sample: TimeSeriesSample) -> dict[str, Any]:
        ...
        window_points, estimated_period = analysis_window_points(
            sample,
            min_points=self.config.reference_min_points,
            max_points=self.config.reference_max_points,
            cycles=self.config.reference_window_cycles,
        )
