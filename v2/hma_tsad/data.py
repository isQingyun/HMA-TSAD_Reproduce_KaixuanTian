...
def reference_catalog(series: np.ndarray, intervals: list[Interval]) -> list[dict[str, Any]]:
...
def select_reference_intervals(
    ...
    """Select diverse consensus reference candidates without reading test labels."""
    ...
    window = (
        max(1, min(length, int(window_points)))
        if window_points is not None
        else max(256, min(length, 2048, int(round(length * window_fraction))))
    )
    if window >= length:
    ...
    candidate_starts = np.linspace(0, length - window, num=min(48, max(5, length // window + 1)), dtype=int)
    ...
    scores = [(float(np.mean(np.abs((features[index] - center) / scale))), start) for index, start in enumerate(starts)]
